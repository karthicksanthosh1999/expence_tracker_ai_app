...
export const POST = async(req:NextRequest,res:NextResponse) => {
    ...
    try {
        const expences = await PRISMA.expences.create({
            data: { amount,category,subject,userId,bankType,paymentDate: new Date(paymentDate) }, 
            include: { user: true}
        })    
        return NextResponse.json({
        ...
        return NextResponse.json({message:"Expences post failed", status: 500, error : error instanceof Error ? error.message : String(error)})
    }
